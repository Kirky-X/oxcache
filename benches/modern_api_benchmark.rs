...
fn bench_l1_operations(c: &mut Criterion) {
    ...
    let cache = rt.block_on(async {
        Cache::<String, String>::new().await.unwrap()
    });
    ...
    c.bench_function("l1_get_or_set", |b| {
        b.to_async(&rt).iter(|| async {
            let key = format!("get_or_set_key_{}", std::time::SystemTime::now().elapsed().unwrap().as_nanos());
            let value: String = cache.get_or(&key, || async {
                Ok(format!("computed_value_{}", std::time::SystemTime::now().elapsed().unwrap().as_nanos()))
            }).await.unwrap();
            black_box(value);
        });
    });
...
fn bench_l1_sizes(c: &mut Criterion) {
    ...
    let cache = rt.block_on(async {
        Cache::<String, Vec<u8>>::new().await.unwrap()
    });
...
fn bench_l1_concurrent(c: &mut Criterion) {
    ...
    for &concurrency in [1, 10, 50, 100].iter() {
        group.bench_with_input(&format!("concurrent_{}", concurrency), concurrency, |b, &concurrency| {
            b.to_async(&rt).iter(|| async {
...
fn bench_l1_batch(c: &mut Criterion) {
    ...
    for &batch_size in [10, 50, 100, 500].iter() {
        group.bench_with_input(&format!("batch_size_{}", batch_size), batch_size, |b, &batch_size| {
            b.to_async(&rt).iter(|| async {
