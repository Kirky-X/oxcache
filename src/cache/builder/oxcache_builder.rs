...
use crate::cache::builder::sorter::BackendSorter;
...
impl OxCacheBuilder {
    ...
    pub async fn redis(connection_string: &str) -> Result<Self> {
        let redis = crate::backend::memory::RedisBackend::new(connection_string).await?;
    ...
    pub async fn tiered(memory_capacity: u64, redis_connection: &str) -> Result<Self> {
        let moka = crate::backend::MokaMemoryBackend::builder()
            ...
            .build();
        let redis = crate::backend::memory::RedisBackend::new(redis_connection).await?;
