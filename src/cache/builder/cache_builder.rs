...
enum InternalBackendConfig {
    ...
    Redis {
        ...
        mode: crate::backend::memory::RedisMode,
    },
    /// 分层后端配置
    #[cfg(feature = "redis")]
    Tiered {
        ...
        l2_mode: crate::backend::memory::RedisMode,
    },
    /// 预构建的后端实例
    ...
    V: crate::core::traits::Cacheable,
{
    ...
    pub fn tiered(mut self, l1_capacity: u64, l2_connection_string: impl Into<String>) -> Self {
        self.backend_config = Some(InternalBackendConfig::Tiered {
            ...
            l2_mode: crate::backend::memory::RedisMode::Standalone,
        });
        self
    }
    ...
    pub async fn build(self) -> Result<Cache<K, V>> {
        let backend = match self.backend_config {
            ...
            Some(InternalBackendConfig::Redis {
                ...
                let backend = crate::backend::memory::RedisBackend::builder()
            ...
            Some(InternalBackendConfig::Tiered {
                ...
                let l2 = crate::backend::memory::RedisBackend::builder()
...
fn backend_config_from_unified_config_with_service(
    config: &crate::core::confers_config::UnifiedConfig,
    service_config: &crate::core::confers_config::ServiceConfig,
) -> std::result::Result<InternalBackendConfig, crate::error::CacheError> {
    let capacity = config
        .backend
        .l1_options()
