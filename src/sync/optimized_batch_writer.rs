...
impl OptimizedBufferEntry {
    fn new(operation: BatchOperation, priority: u8) -> Self {
        Self {
            operation,
...
pub struct OptimizedBatchWriter {
    ...
    /// 优先级队列（用于快速获取高优先级条目）
    priority_queue: Arc<RwLock<Vec<(String, u8, Instant)>>>,
    /// L2缓存后端
    l2: Arc<L2Backend>,
    /// WAL管理器
    wal: Arc<WalManager>,
...
impl OptimizedBatchWriter {
    ...
    pub async fn stop(&self) {
        ...
        // 等待缓冲区清空
        let mut attempts = 0;
        while self.buffer.len() > 0 && attempts < 10 {
    ...
    pub async fn enqueue_operation(
        ...
        if self.config.enable_wal {
            self.wal.write_entry(&key, &self.serialize_operation(&operation)).await?;
        }
    ...
    pub async fn batch_set(
        ...
        self.enqueue_operation(BatchOperation::Set { key, value, ttl }, priority).await
    }

    /// 批量删除（便捷方法）
    pub async fn batch_delete(&self, key: String, priority: u8) -> Result<()> {
        self.enqueue_operation(BatchOperation::Delete { key }, priority).await
    }
    ...
    async fn flush_batch(
            ...
            while !queue.is_empty() && batch.len() < config.base.max_batch_size {
                if let Some((key, priority, _)) = queue.pop() {
                    if let Some(entry) = buffer.get(&key) {
                        ...
                        if batch_size + operation_size > 1024 * 1024 { // 1MB限制
                            queue.push((key, priority, Instant::now()));
                            break;
                        }
        ...
        for (key, operation) in &batch {
            match &operation {
                BatchOperation::Set { value, .. } => {
                    set_operations.push((key.clone(), value.clone()));
        ...
        if !set_operations.is_empty() {
            match l2.pipeline_set_batch(set_operations.clone()).await {
                Ok(_) => {
                    ...
                    stats.successful_operations.fetch_add(set_operations.len() as u64, Ordering::Relaxed);
                }
                Err(e) => {
                                ...
                                if retry_count < config.max_retry_count {
                                    ...
                                    tokio::spawn({
                                        ...
                                        async move {
                                            ...
                                            priority_queue.write().await.push((key.clone(), 255, Instant::now()));
                                        }
                                    });
                                } else {
                                    stats.dropped_operations.fetch_add(1, Ordering::Relaxed);
        ...
        stats.total_bytes_written.fetch_add(total_bytes as u64, Ordering::Relaxed);
        
        let avg_batch_size = (stats.total_operations.load(Ordering::Relaxed) as f64 / stats.batch_count.load(Ordering::Relaxed) as f64 * 100.0) as u64;
        stats.average_batch_size.store(avg_batch_size, Ordering::Relaxed);

        // 更新指标
        Self::update_metrics(buffer, service_name);
    ...
    fn update_metrics(&self) {
        ...
        crate::metrics::GLOBAL_METRICS.set_wal_size("optimized_batch_buffer", self.buffer.len());
    }

    /// 报告统计信息
    async fn report_stats(stats: &BatchWriterStats) {
        ...
        let avg_bytes_per_op = if success > 0 { total_bytes as f64 / success as f64 } else { 0.0 };
        ...
        // 更新全局指标
        crate::metrics::GLOBAL_METRICS.set_batch_success_rate(success_rate);
        crate::metrics::GLOBAL_METRICS.set_batch_throughput(success as f64 / 60.0); // ops/sec
    }
    ...
    pub fn get_stats(&self) -> BatchWriterStats {
        BatchWriterStats {
            ...
            retried_operations: AtomicU64::new(self.stats.retried_operations.load(Ordering::Relaxed)),
            dropped_operations: AtomicU64::new(self.stats.dropped_operations.load(Ordering::Relaxed)),
...
mod tests {
    ...
    fn test_batch_operation_size() {
        ...
        assert_eq!(OptimizedBatchWriter::estimate_operation_size(&set_op), 7); // "key" + "value"

        let delete_op = BatchOperation::Delete {
            key: "key".to_string(),
        };
        assert_eq!(OptimizedBatchWriter::estimate_operation_size(&delete_op), 3); // "key"
    }
