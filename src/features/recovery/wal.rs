...
use crate::backend::storage::{is_test_connection_string, normalize_connection_string};
#[cfg(feature = "wal-recovery")]
        ...
        let raw_connection_string = if is_test {
            ...
            if service_name.contains('/') || service_name.contains('\\') || service_name.contains("..") {
                return Err(crate::error::CacheError::InvalidInput(
                    "service_name 包含非法路径字符".to_string(),
                ));
            }
            let wal_file = format!("{}_wal.db", service_name);
            let wal_path = std::env::current_dir()?.join(&wal_file);
