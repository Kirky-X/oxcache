...
mod redis_integration_tests {
    ...
    async fn test_redis_connection() {
        ...
        let result = backend.set("test:key", b"test_value".to_vec(), Some(Duration::from_secs(60))).await;
...
🎉 All Redis integration tests passed!");
    ...
    async fn test_redis_ttl() {
        ...
        let result = backend.set("ttl:test", b"value".to_vec(), Some(Duration::from_secs(2))).await;
        assert!(result.is_ok());
