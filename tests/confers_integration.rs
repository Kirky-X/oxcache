...
mod tests {
    ...
    ) -> Arc<dyn confers::ConfersConfig> {
        let mut config_map = HashMap::new();
        for (key, value) in pairs {
            config_map.insert(key.to_string(), value);
        }
        Arc::new(config_map) as Arc<dyn confers::ConfersConfig>
    }

    #[tokio::test]
    async fn test_cache_with_confers_memory_backend() {
        let config = create_test_config(vec![
            ("oxcache.backend", json!("memory")),
            ("oxcache.capacity", json!(5000)),
        ]);

        let cache: Cache<String, String> = Cache::with_confers(config.clone())
            .await
    ...
    async fn test_cache_builder_with_confers() {
        let config = create_test_config(vec![
            ("oxcache.backend", json!("memory")),
            ("oxcache.ttl", json!(3600)),
            ("oxcache.capacity", json!(1000)),
        ]);
    ...
    async fn test_cache_builder_manual_override_confers() {
        let config = create_test_config(vec![
            ("oxcache.backend", json!("memory")),
            ("oxcache.ttl", json!(3600)), // confers中的TTL
        ]);
    ...
    async fn test_cache_with_confers_default_memory() {
        ...
        let config = create_test_config(vec![("oxcache.capacity", json!(2000))]);
    ...
    async fn test_backend_builder_with_confers_memory() {
        let config = create_test_config(vec![
            ("oxcache.backend", json!("memory")),
            ("oxcache.capacity", json!(3000)),
            ("oxcache.ttl", json!(1800)),
        ]);
    ...
    async fn test_backend_builder_with_confers_tiered() {
        ...
        {
            let config = create_test_config(vec![
                ("oxcache.backend", json!("tiered")),
                ("oxcache.tiered.l1_capacity", json!(5000)),
                // Redis URL是必需的，但测试环境可能没有Redis
                // ("oxcache.redis.url", json!("redis://localhost:6379")),
            ]);
        ...
        #[cfg(not(feature = "redis"))]
        {
            ...
            let config = create_test_config(vec![("oxcache.backend", json!("tiered"))]);
    ...
    async fn test_confers_config_priority() {
        ...
        let config = create_test_config(vec![
            ("oxcache.backend", json!("memory")),
            ("oxcache.ttl", json!(1000)),      // confers中是1000秒
            ("oxcache.capacity", json!(1000)), // confers中是1000
        ]);
