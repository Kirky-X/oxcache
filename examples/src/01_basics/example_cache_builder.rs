...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 1. 基本构建
    println!("--- 1. 基本构建 ---");
    let _cache1: Cache<String, Product> = CacheBuilder::default()
        .build()
        .await?;
    println!("  ✓ 创建基本缓存");
    ...
    // 2. 设置容量
    println!("\n--- 2. 设置容量 ---");
    let _cache2: Cache<String, Product> = CacheBuilder::default()
        .capacity(10000)
        .build()
        .await?;
    ...
    // 8. 带 TTL 的设置
    println!("\n--- 8. 带 TTL 的设置 ---");
    cache.set_with_ttl(&"product:2".to_string(), &Product {
        id: 2,
